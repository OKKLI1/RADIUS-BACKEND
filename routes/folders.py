...
def ensure_schema():
    ...
    if _schema_ready:
        return
    execute("ALTER TABLE axio_folders ADD COLUMN IF NOT EXISTS vlan_id INT NULL")
    _schema_ready = True
...
def normalize_vlan(vlan_id: Optional[int]):
    ...
    if vlan_id < 1 or vlan_id > 4094:
        raise HTTPException(400, "La VLAN debe estar entre 1 y 4094.")
    return vlan_id
...
def list_folders(payload: dict = Depends(require_admin)):
    """
...
def create_folder(data: FolderCreate, payload: dict = Depends(require_admin)):
    ensure_schema()
    if not data.name.strip():
        raise HTTPException(400, "El nombre no puede estar vacio.")
    vlan_id = normalize_vlan(data.vlan_id)
    if data.parent_id:
    ...
    folder_id = execute(
        "INSERT INTO axio_folders (name, parent_id, vlan_id, created_by, created_at) VALUES (%s, %s, %s, %s, %s)",
        (data.name.strip(), data.parent_id, vlan_id, payload.get("sub"), datetime.utcnow())
    )
    return {"ok": True, "id": folder_id, "name": data.name.strip(), "parent_id": data.parent_id, "vlan_id": vlan_id}


@router.put("/{folder_id}", summary="Actualizar carpeta")
def rename_folder(folder_id: int, data: FolderUpdate,
    ...
    execute("UPDATE axio_folders SET name = %s, vlan_id = %s WHERE id = %s",
            (data.name.strip(), vlan_id, folder_id))
    return {"ok": True, "vlan_id": vlan_id}


@router.delete("/{folder_id}", summary="Eliminar carpeta (y subcarpetas en cascada)")
def delete_folder(folder_id: int, payload: dict = Depends(require_admin)):
    ensure_schema()
    existing = query("SELECT id FROM axio_folders WHERE id = %s", (folder_id,), fetchone=True)
    if not existing:
        raise HTTPException(404, "Carpeta no encontrada.")
...
def get_folder_macs(folder_id: int, payload: dict = Depends(require_admin)):
    ensure_schema()
    rows = query(
        """SELECT
               reg.id,
...
def get_mac_folders(registry_id: int, payload: dict = Depends(require_admin)):
    ensure_schema()
    ...
    if not reg:
        return []
    rows = query(
        """SELECT f.id, f.name, f.parent_id, f.vlan_id
           FROM axio_mac_folders mf
